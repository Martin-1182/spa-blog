<template>
	<div class="card-columns">
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/200?random=1"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator" />
				<p class="card-text">
					Lorem, ipsum dolor sit amet consectetur adipisicing elit.
					Odit hic rerum sint dignissimos delectus eius in aperiam
					incidunt laborum cumque!
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
		</div>
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/300?random=2"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator" />
				<p class="card-text">
					Lorem ipsum dolor sit, amet consectetur adipisicing elit.
					Necessitatibus labore reprehenderit temporibus eligendi a
					fugiat. Eaque, aut accusantium. Obcaecati, eaque.
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
		</div>
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/150?random=3"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator" />
				<p class="card-text">
					Lorem ipsum dolor sit amet consectetur adipisicing elit.
					Voluptatum, autem animi. Nemo iste unde incidunt ut libero
					et in veritatis!
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
		</div>
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/300?random=4"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator" />
				<p class="card-text">
					Lorem ipsum dolor sit, amet consectetur adipisicing elit.
					Necessitatibus labore reprehenderit temporibus eligendi a
					fugiat. Eaque, aut accusantium. Obcaecati, eaque.
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
		</div>
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/300?random=5"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator" />
				<p class="card-text">
					Lorem ipsum dolor sit, amet consectetur adipisicing elit.
					Necessitatibus labore reprehenderit temporibus eligendi a
					fugiat. Eaque, aut accusantium. Obcaecati, eaque.
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
		</div>
		<div class="card">
			<img
				class="card-img-top mb-2"
				src="https://picsum.photos/200/150?random=6"
				alt="Card image cap"
			/>
			<router-link to="" class="tag text-uppercase">Travel</router-link>
			<div class="card-body">
				<router-link to="/show">
					<h4 class="card-title">
						Charming Evening Field
					</h4>
				</router-link>
				<hr class="separator " />
				<p class="card-text">
					Lorem ipsum dolor sit, amet consectetur adipisicing elit.
					Necessitatibus labore reprehenderit temporibus eligendi a
					fugiat. Eaque, aut accusantium. Obcaecati, eaque.
				</p>
				<router-link to="" class="card-text">
					<small class="text-muted">Read More</small>
				</router-link>
			</div>
			<div class="card">
				<img
					class="card-img-top mb-2"
					src="https://picsum.photos/200/200?random=28"
					alt="Card image cap"
				/>
				<router-link to="" class="tag text-uppercase"
					>Travel</router-link
				>
				<div class="card-body">
					<router-link to="/show">
						<h4 class="card-title">
							Charming Evening Field
						</h4>
					</router-link>
					<hr class="separator" />
					<p class="card-text">
						Lorem, ipsum dolor sit amet consectetur adipisicing
						elit. Odit hic rerum sint dignissimos delectus eius in
						aperiam incidunt laborum cumque!
					</p>
					<router-link to="" class="card-text">
						<small class="text-muted">Read More</small>
					</router-link>
				</div>
			</div>
			<div class="card">
				<img
					class="card-img-top mb-2"
					src="https://picsum.photos/200/300?random=29"
					alt="Card image cap"
				/>
				<router-link to="" class="tag text-uppercase"
					>Travel</router-link
				>
				<div class="card-body">
					<router-link to="/show">
						<h4 class="card-title">
							Charming Evening Field
						</h4>
					</router-link>
					<hr class="separator" />
					<p class="card-text">
						Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Necessitatibus labore reprehenderit temporibus
						eligendi a fugiat. Eaque, aut accusantium. Obcaecati,
						eaque.
					</p>
					<router-link to="" class="card-text">
						<small class="text-muted">Read More</small>
					</router-link>
				</div>
			</div>
			<div class="card">
				<img
					class="card-img-top mb-2"
					src="https://picsum.photos/200/150?random=30"
					alt="Card image cap"
				/>
				<router-link to="" class="tag text-uppercase"
					>Travel</router-link
				>
				<div class="card-body">
					<router-link to="/show">
						<h4 class="card-title">
							Charming Evening Field
						</h4>
					</router-link>
					<hr class="separator" />
					<p class="card-text">
						Lorem ipsum dolor sit amet consectetur adipisicing elit.
						Voluptatum, autem animi. Nemo iste unde incidunt ut
						libero et in veritatis!
					</p>
					<router-link to="" class="card-text">
						<small class="text-muted">Read More</small>
					</router-link>
				</div>
			</div>
		</div>
	</div>
</template>

<script>
export default {}
</script>

<style lang="scss" scoped></style>
